...
#
# Requires: output/01-transcript.md (from pipeline.sh)
# Optional: output/03-outline.md (your chosen/edited outline)
#
...
if [ ! -f "$OUTPUT_DIR/01-transcript.md" ]; then
  echo "Error: $OUTPUT_DIR/01-transcript.md not found. Run pipeline.sh first."
  exit 1
fi
...
OUTLINE_JSON="null"
if [ -f "$OUTPUT_DIR/03-outline.md" ]; then
  OUTLINE=$(cat "$OUTPUT_DIR/03-outline.md")
  OUTLINE_JSON=$(printf '%s' "$OUTLINE" | python3 -c 'import json,sys; print(json.dumps(sys.stdin.read()))')
  echo "Using outline from $OUTPUT_DIR/03-outline.md"
else
...
ntn workers exec generateDraft $LOCAL_FLAG \
  ...
  > "$OUTPUT_DIR/04-draft.md"
python3 prettify-files.py "$OUTPUT_DIR/04-draft.md"

echo ""
echo "============================================"
echo "Review the draft in $OUTPUT_DIR/04-draft.md"
echo ""
echo "To revise: create $OUTPUT_DIR/draft-feedback.md with your notes,"
echo "then run: ./pipeline-revise.sh [subject_name]"
echo ""
